...
class LoadRefs(object):
    def __init__(self, data_config, quality_json):
        self.data_config = data_config
        self.cams_order = data_config['cams']  # 固定顺序
    ...
    def __call__(self, results):
        ...
        if sat_bytes is not None:
            sat_img = Image.open(io.BytesIO(sat_bytes)).convert('RGB')   # input_size
            ...
            results['satellite_img'] = torch.zeros((3, fH, fW), dtype=torch.float32)
        ...
        if sample_token is not None and getattr(self, "quality_map", None) is not None:
            q = int(self.quality_map.get(sample_token, -1))
        else:
            q = -1
        results['ref_quality'] = torch.tensor([[int(q)]], dtype=torch.int8)
