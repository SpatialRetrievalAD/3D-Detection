...
def add_ann_adj_info(extra_tag):
    ...
    dataroot = 'xxx/goodet/nuscenes'           # need to change
    nuscenes = NuScenes(nuscenes_version, dataroot)
    for set in ['train', 'val']:
        dataset = pickle.load(
            open('xxx/goodet/nuscenes/%s_infos_%s.pkl' % (extra_tag, set), 'rb'))   # need to change
        ...
        with open('xxx/nuscenes/%s_infos_%s.pkl' % (extra_tag, set),    # need to change
                  'wb') as fid:
...
if __name__ == '__main__':
    ...
    # version = 'v1.0-test'
    root_path = 'xxx/nuscenes'   # need to change
    ...
    # if ONLY BEVDet, no need of bellow;
