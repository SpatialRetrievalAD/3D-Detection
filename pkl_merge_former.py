...
# geoext 生成的 pkl（输入）
STREETVIEW_PKL   = 'data/nuscenes/streetview-400225geoext.pkl'
SATELLITE_PKL    = 'data/nuscenes/satellite_400400geoext.pkl'
...
def _merge_one(bevdet_pkl_path, out_suffix, nusc, stv_db, sat_db, geoext_root_hint=None):
    ...
    # geoext 根目录（尽量从条目里猜；必要时用 geoext_root_hint）
    ...
    for idx, info in enumerate(infos):
        # 1) streetview：写 img2lidar + img_bytes + E/K（带兜底）
        stv_img2lidar = {}
        stv_img_bytes = {}
        stv_K = {}
        stv_E_cam2ego = {}
    ...
    for idx, info in enumerate(infos):
        ...
        # 2) satellite：写 pix2lidar + img_bytes（保持不变）
...
if __name__ == '__main__':
    if os.path.abspath(STREETVIEW_PKL) == os.path.abspath(SATELLITE_PKL):
        print('[WARN] STREETVIEW_PKL 和 SATELLITE_PKL 指向了同一个文件，请确认路径是否写错。')
    ...
    _merge_one(TRAIN_PKL, OUT_SUFFIX, nusc, stv_db, sat_db, GEOEXT_ROOT_HINT)
    _merge_one(VAL_PKL,   OUT_SUFFIX, nusc, stv_db, sat_db, GEOEXT_ROOT_HINT)
